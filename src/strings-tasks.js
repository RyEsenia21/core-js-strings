...
function removeFirstOccurrences(str, value) {
  ...
  if (i !== -1) {
    ...
    const end = str.substring(i + value.length);
    return start + end;
  }
  return str;
}
...
function sumOfCodes(str) {
  if (typeof str === 'undefined' || str === 'NaN' || typeof str !== 'string') {
    return 0;
  }
  let sum = 0;
...
function countVowels(str) {
  ...
  const isVowel = (sum) => vowels.includes(sum);
  const vowelArray = str.split('').filter(isVowel);
...
function isPalindrome(str) {
  const polindrom = str.toLowerCase().replace(/[^a-z]/g, '');
...
function reverseWords(str) {
  ...
  const result = words.map((word) => reverse(word));
  return result.join(' ');
}
...
function extractEmails(str) {
  const emails = str.split(';').filter((item) => item.includes('@'));
  return emails;
}
